...
show_help() {
  ...
  echo "  --kubectl-path VALUE       Specify the path to kubectl binary (REQUIRED)"
  echo
  ...
  echo "  --s3-endpoint-url VALUE   Specify S3 endpoint URL for tests (REQUIRED)"
  ...
  echo "  --bucket-prefix VALUE     Specify prefix for S3 buckets (default: e2e-test)"
  echo "  --additional-args VALUE   Additional arguments to pass to Go tests"
  echo
  ...
  echo
  echo "Examples:"
  echo "  $0 install --kubectl-path=/usr/local/bin/kubectl --endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx"
  echo "  $0 install --namespace custom-namespace --kubectl-path=/usr/local/bin/kubectl --endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx"
  echo "  $0 install --image-tag v1.14.0 --kubectl-path=/usr/local/bin/kubectl --endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx"
  echo "  $0 install --image-repository myrepo/csi-driver --kubectl-path=/usr/local/bin/kubectl --endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx"
  echo "  $0 install --validate-s3 --kubectl-path=/usr/local/bin/kubectl --endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx"
  echo "  $0 test --kubectl-path=/usr/local/bin/kubectl --s3-endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx  # Run all tests including Go-based e2e tests"
  echo "  $0 test --namespace custom-namespace --kubectl-path=/usr/local/bin/kubectl --s3-endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx    # Run tests in a custom namespace"
  echo "  $0 test --skip-go-tests --kubectl-path=/usr/local/bin/kubectl                 # Run only basic verification tests"
  echo "  $0 go-test --kubectl-path=/usr/local/bin/kubectl --s3-endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx                              # Run Go tests directly (skips verification)"
  echo "  $0 all --kubectl-path=/usr/local/bin/kubectl --s3-endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx                              # Install driver and run tests"
  echo "  $0 all --namespace custom-namespace --kubectl-path=/usr/local/bin/kubectl --s3-endpoint-url https://s3.example.com --access-key-id AKIAXXXXXXXX --secret-access-key xxxxxxxx     # Install driver and run tests in a custom namespace"
  echo "  $0 uninstall --kubectl-path=/usr/local/bin/kubectl                            # Uninstall driver from kube-system namespace"
  echo "  $0 uninstall --namespace custom-namespace --kubectl-path=/usr/local/bin/kubectl  # Uninstall driver from a custom namespace"
  echo "  $0 uninstall --namespace custom-namespace --delete-ns --kubectl-path=/usr/local/bin/kubectl  # Uninstall driver and delete custom namespace"
  echo "  $0 uninstall --force --kubectl-path=/usr/local/bin/kubectl                    # Force delete all resources"
  echo "  $0 help                                 # Show this help message"
  echo
  echo "For advanced filtering of Go tests, use 'go test' directly in the tests/e2e-tests directory:"
...
parse_install_parameters() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --namespace)
        ...
        ;;
      --kubectl-path)
        params="$params --kubectl-path $2"
        has_kubectl_path=true
        shift 2
        ;;
      --image-tag)
        IMAGE_TAG="$2"
  ...
  if [ "$has_kubectl_path" = false ]; then
...
parse_test_parameters() {
  local params=""
  local has_kubectl_path=false
  local has_s3_endpoint_url=false
  local has_access_key_id=false
  local has_secret_access_key=false
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --junit-report)
        ...
        ;;
      --s3-endpoint-url)
        params="$params --s3-endpoint-url $2"
        has_s3_endpoint_url=true
        shift 2
        ;;
      --access-key-id)
        params="$params --access-key-id $2"
  ...
  if [[ ! "$params" == *"--skip-go-tests"* ]]; then
    if [ "$has_s3_endpoint_url" = false ]; then
      error "Missing required parameter: --s3-endpoint-url"
      show_help
      exit 1
    fi
