...
if [[ "${ACTION}" == "create_cluster" ]]; then
  ...
  KUBECONFIG=${KUBECONFIG} ginkgo -p -vv -timeout 60m -- --bucket-region=${REGION} --commit-id=${TAG} --bucket-prefix=${CLUSTER_NAME} --imds-available=false
  EXIT_CODE=$?
  print_cluster_info
  exit $EXIT_CODE
  ...
  set +e
  pushd tests/e2e-kubernetes
  KUBECONFIG=${KUBECONFIG} HOST_IP=${HOST_IP} go test -ginkgo.vv --bucket-region=${REGION} --commit-id=${TAG} --bucket-prefix=${CLUSTER_NAME} --performance=true --imds-available=false
  EXIT_CODE=$?
  print_cluster_info
  popd
