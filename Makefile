...
csi-install:
	...
	./tests/e2e-tests/scripts/run.sh install $$INSTALL_ARGS
...
csi-uninstall:
	...
	./tests/e2e-tests/scripts/run.sh uninstall $$UNINSTALL_ARGS
...
e2e-tests:
	...
	./tests/e2e-tests/scripts/run.sh test $$TEST_ARGS
...
e2e-tests-go:
	...
	./tests/e2e-tests/scripts/run.sh go-test $$TEST_ARGS
...
e2e-tests-all:
	@if [ -z "$(S3_ENDPOINT_URL)" ]; then \
		echo "Error: S3_ENDPOINT_URL is required. Please provide it with 'make S3_ENDPOINT_URL=https://your-s3-endpoint.com e2e-tests-all'"; \
		exit 1; \
	fi; \
	if [ -z "$(ACCESS_KEY_ID)" ]; then \
		echo "Error: ACCESS_KEY_ID is required. Please provide it with 'make ACCESS_KEY_ID=your_access_key e2e-tests-all'"; \
		exit 1; \
	fi; \
	if [ -z "$(SECRET_ACCESS_KEY)" ]; then \
