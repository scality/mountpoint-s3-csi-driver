...
sequenceDiagram
    ...
    Controller->>S3: Create S3 bucket via S3 API
    ...
    alt Reclaim Policy: Delete
        ...
        Provisioner->>Controller: DeleteVolume RPC via Unix socket
        Controller->>S3: Delete S3 bucket (only if empty, if not empty the rest of the flow continues)
        S3-->>Controller: Bucket deleted (if empty)
        Controller-->>Provisioner: DeleteVolume response
...
| 2.4 | CSI Controller creates S3 bucket using credentials from secrets or driver-level config | S3 bucket created |
...
| 7.2a | If Delete: Provisioner calls DeleteVolume RPC, Controller deletes S3 bucket and all data | Storage fully removed |
