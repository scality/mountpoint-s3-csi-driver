...
DRIVER_POD=$(kubectl get pods -n kube-system --field-selector spec.nodeName=${NODE_NAME} -o=custom-columns=NAME:.metadata.name | grep scality-s3-csi-node)
kubectl logs ${DRIVER_POD} -n kube-system --container s3-plugin
```

## Mountpoint Logs

Mountpoint logs are written in a node which your application pods are running. The location of the logs may vary by your operating systems, but they usually are written to host's systemd journal.
To fetch these logs, first you need to find the pod UID and node name for the pod.
...
Next, connect to the Kubernetes node with the name discovered in the previous step.
If you are using a cloud provider with worker nodes, you can find more details about how to connect to your nodes in your cloud provider's documentation.
...
If Mountpoint is no longer running since it was unable to mount or possibly died later, there may no longer be an active process to search for.
If a value for `MOUNT_PID` was emitted meaning that the Mountpoint process is still running, note that down.
