...
Features specific to AWS services other than S3, such as S3 Express One Zone, Glacier, or Archive storage classes, are not supported or relevant.
...
3. When these tenets conflict with POSIX requirements, Mountpoint fails early and explicitly.
   It will cause applications to fail with IO errors rather than silently accept operations that will never succeed, such as extended attributes.
...
- Multiple readers can access the same object; only one writer at a time.
