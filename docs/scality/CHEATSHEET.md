...
```bash
make e2e-tests CSI_NAMESPACE=custom-namespace
```

### Run Only Basic Verification Tests (Skip Go Tests)
```bash
make e2e-tests-verify CSI_NAMESPACE=custom-namespace
```
...
```bash
make e2e-tests-go CSI_NAMESPACE=custom-namespace
```

### Advanced Testing with Go Test (for filtering tests)
```bash
# Go to the tests directory
cd tests/e2e-tests/e2e-tests
